<template>
	<div class="card-home-enterprise">
		<stm-nav></stm-nav>
...
export default {
	...
	onLoad(option) {
		...
		var _this = this;
		wx.pro.request({
			url:`${configs.card.apiBaseUrl}api/user/cardfirm/`+option.id,
			method: 'GET',
			header: {
		...
		.then(d => {
			if(d.statusCode == 200){
				_this.imgs = d.data[0].img.split(',')
				_this.img_url = d.data[0].bill.split(',')
				_this.desc = d.data[0].desc ? d.data[0].desc :''
